...
class SARSA_lambda:
    ...
    def update(self, curr_state, curr_action, reward, next_state, next_action, E, Q, alpha, gamma):
        ...
        Performs SARSA lambda update to update action value function Q  
        '''
    ...
    def sarsa_lambda(self, gamma, alpha, epsilon, episodes, lambda_val, env, goal):
        ...
        for episode in range(episodes):
            ...
            # Initializing eligibility traces for each states, action
...
#     lambda_vals = [0]
    ...
    for p in range (len(lambda_vals)):
        ...
        for q in range(len(goal_positions)):
            ...
            # To get the average rewards at each episode and average number of steps in each episode for a lambda and a goal
            ...
            for i in range(12):
                for j in range(12):
                    for k in range(len(data)):
                        # data[k] contains the returned value of kth experiment
            ...
            # Storing average reward and average steps all goals and lambdas 
            com_rew.append(avg)
            com_stp.append(stp)
    ...
    # Selecting all the goal A's    
    x = np.arange(0,len(com_rew),3, dtype = int)

    sr = SARSA_lambda()
    
    # To store all the rewards and steps for all lambda's for goal 'A'
    rew_val = []
    stp_val = []
