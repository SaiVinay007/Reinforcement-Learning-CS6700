...
class SARSA:
    ...
    def plot_sarsa(self, avg_reward, steps, episodes, goal):
        '''
        ...
        # Labelling the plot
        fig1.title.set_text('SARSA : Average reward at each episode for 50 experiments for goal : '+ goal)
        fig1.set_ylabel('Average Reward')
        fig1.set_xlabel('episodes')
        fig1.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        # Labelling the plot
        fig2.title.set_text('SARSA : Average steps at each episode for 50 experiments : '+ goal)
...
if __name__ == '__main__':
    ...
    for x in range(len(goal_positions)):
        ...
        data = [pool.apply(sr.sarsa,args=(gamma, alpha, epsilon, episodes, gym.make('gym_pdw:pdw-v0'),goal)) for i in range(50)] 
        ...
        # Plot for 
        sr.plot_sarsa(avg, stp, episodes, goal)
